...
// Fungsi untuk mengenkripsi pesan dengan Beaufort Cipher dengan UTF-8 dan hasil heksadesimal
char* encrypt(const char *plaintext, const char *key, long length) {
    ...
    while (i < length) {
        ...
        // Enkripsi dengan Beaufort Cipher: C = K - P mod 256
        unsigned char encryptedChar = (keyChar - ch + 256) % 256;
...
// Fungsi untuk mendekripsi pesan dengan Beaufort Cipher dari hex
char* decrypt(const char *ciphertext, const char *key, long length) {
    ...
    while (i < length) {
        ...
        // Dekripsi dengan Beaufort Cipher: P = K - C mod 256
        unsigned char decryptedChar = (keyChar - encryptedChar + 256) % 256;
...
int main() {
    ...
    const char plaintext[] = "This is a test message for Beaufort encryption and decryption!";
    ...
    // Decryption
    char *decrypted_text = decrypt(ciphertext, key, strlen(ciphertext) / 2); // panjang ciphertext dibagi 2
